...
import { ClipboardList, Eye, FileText, Microscope, Stethoscope } from 'lucide-react';
...
const BreastSignoutMasterclass: React.FC<BreastSignoutMasterclassProps> = ({ onSectionChange, embedded = false }) => {
  const [selectedCaseId, setSelectedCaseId] = useState(curriculum.cases[0]?.id ?? '');
  const [revealedStepCount, setRevealedStepCount] = useState(0);
  const [showReport, setShowReport] = useState(false);

  const selectedCase = curriculum.cases.find((item) => item.id === selectedCaseId) ?? curriculum.cases[0];
  const caseAssets = useMemo(
    () => acquiredAssets.filter((asset) => asset.caseId === selectedCase.id),
    [selectedCase.id]
  );
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              ...
              <div className="text-sm font-semibold text-slate-900">{releaseReadyCases} with local visual evidence</div>
            </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" data-testid={`report-panel-${selectedCase.id}`}>

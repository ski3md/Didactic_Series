...
const batchPath = path.join(repoRoot, 'src/content/materials/abpathApMaterialBatch005.json');
...
const reportPath = path.join(repoRoot, 'reports/abpath_ap_material_batch_005.md');
...
describe('ABPath AP material batch 005', () => {
  ...
  it('materializes the next bounded topic-specific AP source-map slice after batches 001 through 004', () => {
    expect(batch.version).toBe('abpath-ap-material-batch.v1');
    expect(batch.batchId).toBe('abpath-ap-material-batch-005');
  ...
  it('documents category-only exclusion and review guardrails', () => {
    ...
    expect(report).toContain('ABPath AP Material Batch 005');

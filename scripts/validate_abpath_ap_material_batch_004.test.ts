...
const batchPath = path.join(repoRoot, 'src/content/materials/abpathApMaterialBatch004.json');
...
const reportPath = path.join(repoRoot, 'reports/abpath_ap_material_batch_004.md');
...
describe('ABPath AP material batch 004', () => {
  ...
  it('materializes the next bounded topic-specific AP source-map slice after batches 001 through 003', () => {
    expect(batch.version).toBe('abpath-ap-material-batch.v1');
    expect(batch.batchId).toBe('abpath-ap-material-batch-004');
  ...
  it('keeps rows review-queued and carries topic-specific source-map evidence', () => {
    ...
    expect(batch.totals.sourceMapSpecificity.topic_specific).toBe(12);
  ...
  it('documents category-only exclusion and review guardrails', () => {
    ...
    expect(report).toContain('ABPath AP Material Batch 004');
    expect(report).toContain('Category-only source-map rows excluded');
    expect(report).toContain('topic_specific=12');

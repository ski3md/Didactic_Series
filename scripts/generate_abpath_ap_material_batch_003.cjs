...
const outJsonPath = path.join(repoRoot, 'src/content/materials/abpathApMaterialBatch003.json');
const outReportPath = path.join(repoRoot, 'reports/abpath_ap_material_batch_003.md');

const BATCH_ID = 'abpath-ap-material-batch-003';
const BATCH_SIZE = 12;
...
function buildPayload(queue, sourceMap, priorBatches) {
  ...
  return {
    ...
    purpose:
      'Third bounded AP core-resident materialization batch from topic-specific local source-map rows; review-queue only, not promoted teaching truth.',
    selectionCriteria: {
      domain: 'AP',
      ...
      ordering: 'AP local source map order after excluding AP material batches 001 and 002',
    ...
    guardrails: [
      ...
      'Batch 003 may use only topic-specific local source-map rows.',
...
function main() {
  ...
  console.log(`[ABPATH-AP-MATERIAL-BATCH-003] Wrote ${payload.totals.rows} rows to ${path.relative(repoRoot, outJsonPath)}.`);
}

...
const outJsonPath = path.join(repoRoot, 'src/content/materials/abpathApMaterialBatch002.json');
const outReportPath = path.join(repoRoot, 'reports/abpath_ap_material_batch_002.md');

const BATCH_ID = 'abpath-ap-material-batch-002';
const PRIOR_BATCH_ID = 'abpath-ap-material-batch-001';
const BATCH_SIZE = 12;
...
function buildPayload(queue, priorBatch) {
  const priorQueueIds = new Set(priorBatch.rows.map((row) => row.queueId));
  const eligibleRows = queue.entries.filter(
    (entry) =>
      entry.domain === 'AP' &&
  ...
  const rows = eligibleRows
    .filter((entry) => !priorQueueIds.has(entry.id))
    .slice(0, BATCH_SIZE)
    .map(materializeRow);

  if (rows.length !== BATCH_SIZE) {
    throw new Error(`Expected ${BATCH_SIZE} AP P0 rows after ${PRIOR_BATCH_ID}, found ${rows.length}`);
  }
  ...
  return {
    ...
    priorBatch: {
      ...
      excludedQueueIds: Array.from(priorQueueIds),
    },
    purpose:
      'Second bounded AP core-resident materialization batch from the ABPath expansion queue; review-queue only, not promoted teaching truth.',
    selectionCriteria: {
      domain: 'AP',
      ...
      ordering: 'existing expansion queue order after excluding prior AP batch 001 queue IDs',
    ...
    guardrails: [
      ...
      'Batch 002 must not duplicate any queueId from AP material batch 001.',
    ],
    totals: {
      rows: rows.length,
...
function buildReport(payload) {
  return [
    ...
    `- Prior batch excluded: ${payload.priorBatch.batchId} (${payload.priorBatch.excludedQueueIds.length} queue IDs)`,
    ...
    `- Duplicate policy: ${payload.selectionCriteria.duplicatePolicy}`,
    '',
    '## Totals',
    '',
    ...
    '',
    markdownTable(
      ['#', 'Queue ID', 'Subject', 'Kind', 'Title', 'Path hierarchy', 'Source path', 'Required material set', 'Review', 'Promotion'],
...
function main() {
  ...
  const priorBatch = readJson(priorBatchPath);
  const payload = buildPayload(queue, priorBatch);
  ...
  console.log(`[ABPATH-AP-MATERIAL-BATCH-002] Wrote ${payload.totals.rows} rows to ${path.relative(repoRoot, outJsonPath)}.`);
}

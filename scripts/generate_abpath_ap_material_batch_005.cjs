...
const outJsonPath = path.join(repoRoot, 'src/content/materials/abpathApMaterialBatch005.json');
const outReportPath = path.join(repoRoot, 'reports/abpath_ap_material_batch_005.md');

const BATCH_ID = 'abpath-ap-material-batch-005';
const BATCH_SIZE = 12;
...
function buildPayload(queue, sourceMap, priorBatches) {
  ...
  return {
    ...
    purpose:
      'Fifth bounded AP core-resident materialization batch from remaining topic-specific local source-map rows; review-queue only, not promoted teaching truth.',
    selectionCriteria: {
      domain: 'AP',
      ...
      ordering: 'AP local source map order after excluding AP material batches 001, 002, 003, and 004',
    ...
    guardrails: [
      ...
      'Batch 005 may use only remaining topic-specific local source-map rows.',
...
function main() {
  ...
  console.log(`[ABPATH-AP-MATERIAL-BATCH-005] Wrote ${payload.totals.rows} rows to ${path.relative(repoRoot, outJsonPath)}.`);
}

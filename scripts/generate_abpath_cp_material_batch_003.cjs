...
const outJsonPath = path.join(repoRoot, 'src/content/materials/abpathCpMaterialBatch003.json');
const outReportPath = path.join(repoRoot, 'reports/abpath_cp_material_batch_003.md');

const BATCH_ID = 'abpath-cp-material-batch-003';
...
function buildBatch(queue, coverageMap) {
  ...
  return {
    ...
    purpose:
      'Third bounded CP materialization slice from the worked-example coverage map; review-queue only, not promoted teaching truth.',
    selection: {
      ...
      domain: 'CP',
      selectionRationale:
        'First ten unmaterialized CP worked-example coverage rows after CP batches 001 and 002.',
      queueEntryIds: SELECTED_QUEUE_ENTRY_IDS,
      excludedPriorBatchIds: ['abpath-cp-material-batch-001', 'abpath-cp-material-batch-002'],
    },
    totals: {
      rows: rows.length,
...
function main() {
  ...
  console.log(`[ABPATH-CP-MATERIAL-BATCH-003] Wrote ${batch.totals.rows} rows to ${path.relative(repoRoot, outJsonPath)}.`);
}

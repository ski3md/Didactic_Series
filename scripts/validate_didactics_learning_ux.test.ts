...
describe('validate_didactics_learning_ux helpers', () => {
  ...
  it('accepts objective-first hierarchy when core framing and content appear before optional follow-up links', () => {
    const result = evaluateHierarchySemantics({
      ...
      lecturesTsx: `
        ...
        <div>Resume last lecture</div>
        <div>Browse topics</div>
        <div>Open the topic list</div>
        <div>Major topics</div>
        <div>Major topic</div>
    ...
    const result = evaluateHierarchySemantics({
      ...
      expect(result.issues).toEqual(
        expect.arrayContaining([
          ...
          expect.stringContaining('Lecture landing does not clearly expose both a direct resume-or-start CTA and a browse CTA before the topic grid.'),
          expect.stringContaining('Workup landing does not clearly expose both a direct resume-or-start CTA and a browse CTA before the diagnostic-area grid.'),
  ...
  it('accepts reference-route semantics when the reference lane is query-routed and pathology-native', () => {
    const result = evaluateReferenceRouteSemantics({
      ...
      referenceTsx: `
        <div>Start here</div>
        <div>Training level guidance</div>
        <div>Diagnostic focus</div>
    ...
    expect(result.passes).toEqual(
        expect.arrayContaining([
          ...
          expect.stringContaining('Reference Library keeps training-level guidance ordered from diagnostic focus to recognition targets.'),
        ])
      );
  });

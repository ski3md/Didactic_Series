...
const md = [
  ...
  '## Action Buckets',
  '',
  `- Safe duplicate exclusions: ${payload.actionBuckets.safeDuplicateExclusions.join(', ')}`,
  `- Source-map review required: ${payload.actionBuckets.sourceMapReviewRequired.join(', ')}`,
  '',
  '## Duplicate Pairs',
  '',

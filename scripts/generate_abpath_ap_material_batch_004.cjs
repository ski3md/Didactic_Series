...
const outJsonPath = path.join(repoRoot, 'src/content/materials/abpathApMaterialBatch004.json');
const outReportPath = path.join(repoRoot, 'reports/abpath_ap_material_batch_004.md');

const BATCH_ID = 'abpath-ap-material-batch-004';
const BATCH_SIZE = 12;
...
function buildPayload(queue, sourceMap, priorBatches) {
  ...
  return {
    ...
    purpose:
      'Fourth bounded AP core-resident materialization batch from remaining topic-specific local source-map rows; review-queue only, not promoted teaching truth.',
    selectionCriteria: {
      domain: 'AP',
      ...
      ordering: 'AP local source map order after excluding AP material batches 001, 002, and 003',
    ...
    guardrails: [
      ...
      'Batch 004 may use only remaining topic-specific local source-map rows.',
...
function main() {
  ...
  console.log(`[ABPATH-AP-MATERIAL-BATCH-004] Wrote ${payload.totals.rows} rows to ${path.relative(repoRoot, outJsonPath)}.`);
}
